...
class XLerobot(Robot):
    ...
    def __init__(self, config: XLerobotConfig):
        ...
        if config.use_head:
            # head
            bus1_motors["head_motor_1"] = Motor(7, "sts3215", norm_mode_body)
            bus1_motors["head_motor_2"] = Motor(8, "sts3215", norm_mode_body)

        if self.calibration.get("left_arm_shoulder_pan") is not None:
            calibration1 = {
                motor_name: self.calibration.get(motor_name) for motor_name in bus1_motors
            }
        else:
        ...
        if config.use_base:
            # base
            bus2_motors["base_left_wheel"] = Motor(7, "sts3215", MotorNormMode.RANGE_M100_100)
            bus2_motors["base_back_wheel"] = Motor(8, "sts3215", MotorNormMode.RANGE_M100_100)
            bus2_motors["base_right_wheel"] = Motor(9, "sts3215", MotorNormMode.RANGE_M100_100)

        if self.calibration.get("right_arm_shoulder_pan") is not None:
            calibration2 = {
                motor_name: self.calibration.get(motor_name) for motor_name in bus2_motors
            }
        else:
        ...
        # Last successfully captured frame per camera, used to survive transient
        # USB/V4L2 stalls without aborting the whole recording.
    ...
    def _state_ft(self) -> dict[str, type]:
        ...
        if self.config.use_base:
            state_keys += ["x.vel", "y.vel", "theta.vel"]
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        for cam_key, cam in self.cameras.items():
            try:
                self._last_camera_frames[cam_key] = cam.async_read()
            except TimeoutError:
                logger.warning(f"Could not grab initial frame from {cam_key}")
    ...
    def get_camera_observation(self):
        ...
        for cam_key, cam in self.cameras.items():
            start = time.perf_counter()
            try:
                # Short timeout: a stalled camera must not drag the whole 30 Hz
                # record loop down to ~5 Hz (200 ms default). On timeout we reuse
                # the last frame so only the flaky camera freezes.
                obs_dict[cam_key] = cam.async_read(timeout_ms=40)
            except TimeoutError:
                # Transient camera stall (USB/V4L2 underrun): reuse the last known
                # frame instead of aborting the whole recording.
                last = self._last_camera_frames.get(cam_key)
                if last is None:
                    raise
                logger.warning(f"{cam_key} camera timed out; reusing last frame.")
                obs_dict[cam_key] = last
            else:
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.max_relative_target is not None:
            ...
            present_pos_head = (
                self.bus1.sync_read("Present_Position", self.head_motors) if self.head_motors else {}
            )
