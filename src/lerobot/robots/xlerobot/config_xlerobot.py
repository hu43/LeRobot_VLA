...
def xlerobot_cameras_config() -> dict[str, CameraConfig]:
    return {
        "left_wrist": OpenCVCameraConfig(
            index_or_path="/dev/left_wrist_cam", fps=30, width=640, height=480,
            color_mode=ColorMode.RGB, rotation=Cv2Rotation.NO_ROTATION, fourcc="MJPG"
        ),

        "right_wrist": OpenCVCameraConfig(
            index_or_path="/dev/right_wrist_cam", fps=30, width=640, height=480,
            color_mode=ColorMode.RGB, rotation=Cv2Rotation.NO_ROTATION, fourcc="MJPG"
        ),

        "head": OpenCVCameraConfig(
            index_or_path="/dev/head_cam", fps=30, width=640, height=480,
            color_mode=ColorMode.RGB, rotation=Cv2Rotation.NO_ROTATION, fourcc="MJPG"
        ),
    }
...
class XLerobotConfig(RobotConfig):
    ...
    port1: str = "/dev/left_arm"  # port to connect to the bus (so101 + head camera)
    port2: str = "/dev/right_arm"  # port to connect to the bus (same as lekiwi setup)
    disable_torque_on_disconnect: bool = True

    # Set to False when the head motors (IDs 7-8 on port1) are not connected
    use_head: bool = True
    # Set to False when the wheeled base (IDs 7-9 on port2) is not connected
    use_base: bool = True
