#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""采集并校验「头部」摄像头数据（无界面，不弹窗显示图像）。

只读取视频帧、打印帧信息并把图像保存到磁盘，供后续查看/用于数据采集测试。
对应机器人 config 里的摄像头 key: head

注意：当前机器上头部用的是 USB 摄像头（/dev/video4）。如果你之后换用 RealSense
做头部 RGB-D，请把下面的 OpenCVCamera 换成 RealSenseCamera / RealSenseCameraConfig，
并在录制时用 read_depth() / async_read_depth() 额外保存深度图。

用法示例:
    uv run python collect_camera_head.py                        # 存 5 帧 jpg
    uv run python collect_camera_head.py --num-frames 30        # 存 30 帧
    uv run python collect_camera_head.py --video 90             # 再录 90 帧 -> mp4
    uv run python collect_camera_head.py --device /dev/video4   # 换设备
"""
