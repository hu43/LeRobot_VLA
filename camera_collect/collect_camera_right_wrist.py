#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""采集并校验「右腕」摄像头数据（无界面，不弹窗显示图像）。

只读取视频帧、打印帧信息并把图像保存到磁盘，供后续查看/用于数据采集测试。
对应机器人 config 里的摄像头 key: right_wrist

用法示例:
    uv run python collect_camera_right_wrist.py                        # 存 5 帧 jpg
    uv run python collect_camera_right_wrist.py --num-frames 30        # 存 30 帧
    uv run python collect_camera_right_wrist.py --video 90             # 再录 90 帧 -> mp4
    uv run python collect_camera_right_wrist.py --device /dev/video4   # 换设备
"""
...
DEVICE = "/dev/right_wrist_cam"
OUT_DIR = "camera_frames/right_wrist"
