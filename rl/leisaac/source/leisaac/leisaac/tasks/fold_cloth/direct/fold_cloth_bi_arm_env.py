...
class FoldClothBiArmEnv(BiArmTaskDirectEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        approach = mdp.approach_cloth_reward(env=self, sigma=5.0)
        grasp = mdp.grasp_cloth_reward(env=self, dist_threshold=0.12)
        fold = mdp.fold_cloth_reward(env=self, sigma=2.0)
        rest = mdp.rest_pose_reward(env=self)
        coverage_pen = mdp.cloth_coverage_penalty(env=self, fall_height=0.5)
        # weight late-stage rewards higher once approach is satisfied
        grasp_gate = grasp.detach() if isinstance(grasp, torch.Tensor) else float(grasp)
        fold_gate = (fold > 0.3).float().detach() if isinstance(fold, torch.Tensor) else float(fold > 0.3)
        total = (
            0.2 * approach
            + 0.3 * grasp_gate * grasp
            + 1.0 * fold
            + 0.2 * fold_gate * rest
            + 0.5 * coverage_pen
        )
        if not isinstance(total, torch.Tensor):
            total = torch.full((self.num_envs,), float(total), device=self.device)
        return total
