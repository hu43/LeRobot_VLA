...
import numpy as np
import torch
from leisaac.utils.constant import SINGLE_ARM_JOINT_NAMES, BI_ARM_JOINT_NAMES
...
class LeRobotServicePolicyClient(Policy):
    ...
    def __init__(
        ...
        elif task_type in ["bi_so101_state_machine", "bi_so101leader"]:
            lerobot_features["observation.state"] = {
                "dtype": "float32",
                "shape": (12,),
    ...
    def _send_observation(self, observation_dict: dict):
        ...
        if self.task_type == "so101leader":
        ...
        elif self.task_type in ["bi_so101_state_machine", "bi_so101leader"]:
            ...
            for i, joint_name in enumerate(SINGLE_ARM_JOINT_NAMES):
                raw_observation[f"left_{joint_name}.pos"] = left_joint_pos[0, i].item()
                raw_observation[f"right_{joint_name}.pos"] = right_joint_pos[0, i].item()
